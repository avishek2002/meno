...
- 5-7 questions total for a full interview. When an evidence packet exists and anchors `prior_level` and/or `user_sources` (see Preflight), the anchored confirms in Phases 2 and 4 - and the live probe in Phase 2, which still runs, just against the anchor instead of a blind claim - verify observed evidence rather than ask blind, so none of them count against the cap, the same way the Phase 5 confirmation never has; the budget shrinks to 2-3 questions covering goal and time. Track the count; when the cap is reached, move to confirmation with stated defaults for anything unasked.
...
Before question one: if exactly one directory exists under `content/tenants/`, that is the tenant. If none exists, use `content/tenants/main/`. Only if several exist, ask which one - and that question does not count against the interview budget.
...
Then check for an evidence packet: `content/tenants/<tenant>/subjects/evidence-packet.json`. When it exists, read it - an array of `{field, anchor, evidence}` entries, each naming `prior_level` or `user_sources` (produced by `find-subjects`; contract in [docs/specs/subject-finder.md](../../../docs/specs/subject-finder.md)) plus a one-line observed fact and the snapshot field it came from - and hold those anchors for Phases 2 and 4 below. When no packet exists - the ordinary case, true for any learner who names a topic directly rather than arriving from `find-subjects` - proceed exactly as an unmodified full interview; a missing packet is the unremarkable default, not a degraded path.
...
2. **Prior knowledge (1 menu or anchored confirm + 1 live probe).** When the packet carries no `prior_level` anchor, ask the behavioral menu (never touched it / know the vocabulary / built small things / comfortable). When it does, skip the blank menu and state the observed fact instead - "Your workspace shows <anchor> - does that match where you're starting from?" - confirm or adjust, once. Either way, then run one micro-task pitched at the claimed level - explain a term in their own words, predict what a snippet does, spot the odd one out. Adjust the level from the result, and say so plainly and kindly. Precedence, in order: **live probe result > workspace evidence > self-report.** The probe still runs regardless of an anchor, and its result overrides both the anchor and any self-report.
...
4. **Format and own materials (1 question or anchored confirm, skippable with stated default).** Default is text-first lessons. When the packet carries no `user_sources` anchor, ask blind whether they have their own materials (documents, notes, a textbook, work artifacts). When it does, replace the blind ask with a confirm over what was observed - state the anchor and ask whether it should anchor lessons, rather than asking as if nothing is known. Either way, if yes, have them drop files into `content/tenants/<tenant>/sources/` - those become anchor sources during generation.
...
Derive the course slug first: kebab-case of the topic as scoped by the outcome (e.g. "SQL for analytics work" -> `sql-for-analytics`), 2-4 words, shown to the user in the confirmation brief - slugs are stable once created because wikilinks and manifests bind to them. Also classify the course into a domain: pick the single best fit from the closed vocabulary in [content/community/DOMAINS.md](../../../content/community/DOMAINS.md); if genuinely none fits, say so to the user instead of inventing one, and let them pick the closest. Then write `content/tenants/<tenant>/<domain>/<course-slug>/profile.md` exactly per [references/profile-format.md](references/profile-format.md) (frontmatter contract plus four body sections). When Preflight found an evidence packet and its anchors were actually used, set `evidence_packet` in the frontmatter to the packet's path relative to the tenant directory (`subjects/evidence-packet.json`); omit the field entirely when no packet was consumed - that is the ordinary case and the field's absence means exactly that.
