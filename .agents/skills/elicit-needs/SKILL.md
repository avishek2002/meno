...
- Skip any question already answered by what the user said unprompted.
...
3. **Depth x time (2 closed questions).** Depth menu: orient / build something real / work-ready / teach (this sets the Bloom ceiling for every objective downstream; certification goals live in `goal_category: career`, not here). Then time: hours per week and total weeks. If depth times topic clearly exceeds the budget, push back now with two honest options - shallower depth or longer runway - and let them choose. Never silently accept an impossible contract; scope mismatch is the number-one reason self-paced learners quit.
