...
- **Notes hub link** (part of any sweep, and what clears the `Link <course>-notes into the course hub` todo the app files on a course's first saved note): when a course directory contains `<course-slug>-notes.md`, its hub must carry `- [[<course-slug>-notes|My notes]]` under `## My notes`. Add the line if it is missing; the note blocks inside that file are the learner's and the app's (docs/specs/notes.md) - read them if useful, never restructure or regenerate them.
