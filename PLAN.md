...
| 19 | Content accuracy + community slate | (2026-08-05 grill, maintainer-decided; plan in [docs/plans/content-accuracy-and-community.md](docs/plans/content-accuracy-and-community.md).) `generate-module` gains a blocking self-audit - every factual claim traces to a cited source or is fixed/flagged, and every check's answer key is independently re-solved - paired with seeded-fault fixtures and eval scorers that drill the auditor itself. Community packs stay skeletons; a five-pack intern-onboarding slate lands hand-authored via draft-a-pack. Sourcing boundary: the maintainer's private vault is inspiration-only; personally authored harness/skill craft is publishable after scrubbing workplace specifics, enforced by a named review question in the pull request template. |

| 20 | Course groups | (2026-08-06, maintainer-decided; reconciled with the domain layout the same day.) The course list groups in two layers: a course's **domain directory** is the default grouping, costing the learner nothing, and a `groups.yml` registry at the vault root holds the free-form groups they name themselves for what a domain cannot say ("Version Control", "Software Fundamentals"). An explicit group always wins; a course no group claims falls back to its domain; only a course with no domain at all is Ungrouped. Full create/rename/delete/move lives in the app. Membership is deliberately NOT a field on `course.yml`, which is regenerated wholesale from the module manifests. This **extends decision 16 rather than amending decision 14**: `todos.md`'s write discipline (single server writer, atomic replace, `If-Match` guarded) generalizes to any UI-owned tenant registry file, and `groups.yml` is the first other instance. Decision 14 governs *evidence* - who may claim a learner knows something - and no group operation appends a ledger event, touches `mastery.yml`, or edits a manifest, so the seam is untouched. A new app-writable file is legitimate only in that second, organizational class. Group vocabulary stays open, unlike the pack tier's closed `DOMAINS.md`, because one learner curating their own shelf is not a multi-contributor commons - and the closed vocabulary keeps governing the directory layer underneath, so the two coexist without competing: one is where a course *sits*, the other is what the learner *calls* it. **Amended 2026-08-06 (v1.6, maintainer-decided):** the create/rename/delete/move surface in the app is removed - four write routes and an inline manage panel - and `groups.yml` becomes agent-edited and hand-edited only. The explicit layer competes with the domain layer rather than complementing it (a course belongs to exactly one group), so the layer that costs the learner nothing is the one that stays, and the panel was never verified in a browser. Everything else in this decision holds: the file is still parsed, resolved, schema'd and validated, an explicit group still wins over a domain, and the write-authority reading is untouched - the app writing one fewer file can only narrow the seam. The course list gains collapse and filter in the same change, which is the need the manage panel was answering badly. |
| 21 | Pack attribution | (2026-08-06, maintainer-decided.) Every community pack carries a `CONTRIBUTORS.yml` recording who made what at the smallest unit a change touches - pack, course objective, module, planned lesson, anchor source, reference note - with **nearest-ancestor inheritance**, so a single-author pack needs exactly one record and finer records exist only where authorship differs. It is a new file, not `PACK.md` frontmatter: `maintainers` answers "who reviews this" and conflating that with authorship would break one-owner-per-format. Source units key on the url (it survives reordering and re-archiving, which an index does not); lesson units resolve against the module manifest, since packs ship no lesson bodies. Honest limit, recorded rather than engineered around: `by` is self-declared, and append-only is guarded by git history and review, not by validate. |
