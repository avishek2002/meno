...
| 20 | Course groups | (2026-08-06, maintainer-decided.) A tenant's courses are organized by a `groups.yml` registry at the vault root - free-form groups the learner names, each holding course slugs, with anything unclaimed rendering as Ungrouped. Membership is deliberately NOT a field on `course.yml`, which is regenerated wholesale from the module manifests. This **extends decision 16 rather than amending decision 14**: `todos.md`'s write discipline (single server writer, atomic replace, `If-Match` guarded) generalizes to any UI-owned tenant registry file, and `groups.yml` is the first other instance. Decision 14 governs *evidence* - who may claim a learner knows something - and no group operation appends a ledger event, touches `mastery.yml`, or edits a manifest, so the seam is untouched. A new app-writable file is legitimate only in that second, organizational class. Group vocabulary stays open, unlike the pack tier's closed `DOMAINS.md`, because one learner curating their own shelf is not a multi-contributor commons. |
