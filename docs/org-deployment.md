...
[topic-packs/README.md](../topic-packs/README.md); the tenancy boundary in
...
learners), and an organization's `org/` tree - see below - is exactly the kind of content a
fork's public visibility would expose by construction. The fix is a bare-clone-and-push, the
...
This is a different mirror from the one `content/` already gets. [`tools/meno-mirror`](../tools/meno-mirror)
backs up one *learner's* `content/<tenant>/` directory to a private repo that learner owns -
small, per-person, and invisible to everything else. The clone above is one level up: your
org's copy of the *entire base repository plus `org/`*, shared by everyone who works from it.
The two never overlap - `content/` stays gitignored inside the org clone exactly as it does
everywhere else, so nothing about this pattern changes what an individual learner backs up or
how (see [(d)](#d-what-a-learners-clone-keeps-to-itself)).

## (b) The org knowledge base: org/

`org/` is a reserved, downstream-owned root, structurally the same idea as `content/<tenant>/`
one directory up: this repository never creates it, never writes to it, and never ships
anything inside it. Where `content/` belongs to one learner, `org/` belongs to the
organization that deployed this clone - and unlike `content/`, it is meant to be committed.
That is the whole design: it uses the **pack format, verbatim** - the exact shape
[`topic-packs/`](../topic-packs/README.md) already defines, because inventing a second format
for "shared curriculum, but private" would be a second thing to maintain for zero new
capability.
...
org/
  ...
  packs/
    <domain>/<slug>/
      course.yml                     status: draft, no profile field (pre-contract, same as topic-packs/)
...
`tools/validate.ts`'s `checkPacks` already walks `org/` alongside `topic-packs/` when the
directory exists - this is existing plumbing, not new surface: `pack.schema.json`'s `pack`
field pattern and `course.schema.json`'s `derived_from.pack` pattern both already accept
`org/packs/<domain>/<slug>` next to `topic-packs/<domain>/<slug>`
([docs/specs/community.md](specs/community.md)). One difference from the public tier: **org
domains are exempt from `topic-packs/DOMAINS.md`'s closed vocabulary** (`checkPacks` only
checks the domain against `DOMAINS.md` when the pack is *not* under `org/`) - your
...
|---|---|---|
| Upstream community | `topic-packs/<domain>/<slug>/` | every Meno clone, public |
| Org-private | `org/packs/<domain>/<slug>/` | your org's private clone only |
| Tenant-local | `content/<tenant>/<slug>/` (adopted) | one learner, after `extend-meno`'s adopt-a-pack recipe |

An org can contribute a pack upstream by literally moving `org/packs/<domain>/<slug>/` to
`topic-packs/<domain>/<slug>/` on a branch against the public repo (checking the domain
...
`org/notes/` (really `org/packs/<domain>/<slug>/notes/`) works exactly like `topic-packs/`'s
reference notes: ground truth a lesson can cite, never pedagogy, never a check block. Same
schema, same `pack-notes` check, same rule that anything under `org/` is reference **data** to
every skill that reads it, never instructions.
...
role system decides who can push to `org/` and who can merge a pull request. It cannot decide
...
|---|---|---|---|
| KB (knowledge base) admin | curate `org/`, merge pull requests into it, set branch protection | **Maintain** role, listed in a `CODEOWNERS` entry for `org/` | **Maintainer** role |
| Contributor | propose changes to `org/` via pull request; cannot merge without review | **Write** role + a branch-protection rule requiring review before merge | **Developer** role + a protected branch requiring merge-request approval |
| Learner | clone the repo, read everything, never writes `org/` | **Read** role | **Reporter** role |

**A domain-scoped team is not expressible in one repository.** Someone who should curate
`org/packs/data/` but not `org/packs/infrastructure/` cannot be granted that on GitHub or
...
isolation, the two honest options are splitting `org/packs/<domain>/` across separate
...
Branch protection on `org/` (and on the repo generally, for the same reasons the workspace's
...
that flow changes because the clone happens to be an org deployment: `content/<tenant>/`
...
`content/` is gitignored in every clone of this repository, including the org's - nobody
commits it, so it is never part of any push a learner makes back to the org's `origin`
remote, and there is no code path anywhere in this repo that reads one tenant's `content/`
from another context. The only ways progress data ever leaves a learner's machine are things
...
`content/` or `org/`** before merging anything. That refusal is the tool's entire reason to
exist: `content/` and `org/` are reserved downstream-owned roots the base repo never creates
or writes to ([specs/repo-and-tenancy.md](specs/repo-and-tenancy.md)), so a legitimate
upstream change can never touch either path - if one somehow does, that is a signal to stop
and look by hand, not something to merge blind. Ordinary upstream changes merge and then run
`npm run gate`, reporting honestly if it fails.
...
*next* clone, not the last one - keep that in mind for what you put in `org/`.
...
- [specs/durability.md](specs/durability.md) - the per-learner mirror this pattern deliberately
  does not replace.
- [specs/community.md](specs/community.md) - the pack format `org/` reuses verbatim.
- [specs/repo-and-tenancy.md](specs/repo-and-tenancy.md) - the tenancy boundary `org/` extends.
