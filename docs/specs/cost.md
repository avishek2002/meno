...
Paste-ready. The producer types live in `lib/cost.ts`; the wire type lives in
`app/shared/types.ts` and re-exports rather than duplicates, exactly as `InsightsReport` does.

```ts
// lib/cost.ts - the snapshot, produced by computeCost.
...
export interface CourseCost {
   ...
   * rounds below a cent, so a course with real evidence never renders as $0.00 (amended
   * 2026-08-12, adversarial review finding 2 - the first draft's plain rounding could produce a
   * $0.00 course row, which both misrepresents a course that did cost something and violates
   * schemas/cost.schema.json's own exclusiveMinimum on this field).
   */
  cost_usd: number;
 ...
 * Changed from a bare `string` to this shape 2026-08-12 (adversarial review finding NEW-1): the
 * first draft's `no_data: string[]` could not represent two directories sharing a basename
 * without either colliding them or leaving `tools/validate.ts`'s cross-field check comparing the
 * wrong field (basename) against `courses[].dir` - a false positive that rejected a perfectly
 * valid snapshot.
 */
...
export interface CostTotals {
   ...
   * transcript and every transcript on the shared line. Never the size of the source's whole
   * collect() result, which typically spans every project on the machine (amended 2026-08-12,
   * adversarial review finding 4 - the first draft used the raw collect() count here, which on
   * the reference machine read ~45x higher than this tenant's actual attributed activity under a
   * heading that says "cost for <tenant>").
   */
...
export interface CostSnapshot {
  ...
  /** The disclosures the page renders verbatim. See "Disclosures" below. */
  limits: string[];
}
...
export interface CostMeta {
  ...
  prices_as_of: string;
  generated_at: string;
  /** Transcripts the source could not read (CostCollectResult.skipped). Surfaced in limits when
   *  nonzero. Zero when there is no source, or the source reported none. */
  skipped: number;
}

/** The one pure derivation. No clock, no filesystem, no environment. */
export function computeCost(
  transcripts: CostTranscript[],
  courseDirs: string[], // findCourseDirs(tenantDir) output, "<domain>/<slug>" or "<slug>"
  meta: CostMeta,
): CostSnapshot;
```

```ts
// lib/cost-io.ts - the filesystem edges, kept out of the pure function.

/** Vault-relative path of the snapshot: "cost/snapshot.json". */
export const COST_SNAPSHOT_REL: string;
...
/** Write a snapshot atomically (app/server/atomic.ts writeFileAtomic), creating cost/ if needed. */
...
```ts
// app/shared/types.ts - the wire type. Re-export, do not duplicate.
// CostNoDataEntry added 2026-08-12 (adversarial review finding NEW-1) alongside the no_data
// shape change - a component naming that shape directly needs it exported too.
...
Paste-ready, same draft and `$id` convention as `schemas/insights.schema.json`. One deliberate
difference from that file: `additionalProperties` is `false` throughout. `insights.schema.json`
describes frontmatter a human and a skill write, so it is permissive; this describes a file our own
tool produces, where the schema is the contract and an unexpected key means the producer drifted.
Adding a field is therefore a schema edit in the same change, which is the point.

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://github.com/avishek2002/meno/schemas/cost.schema.json",
  "title": "Meno content-cost snapshot",
  "description": "The aggregate cost snapshot written by tools/cost.ts to content/tenants/<tenant>/cost/snapshot.json and read by GET /api/v1/:tenant/cost. Aggregates only: no transcript content of any kind. Shape owned by lib/cost.ts; behavior owned by docs/specs/cost.md. additionalProperties is false on purpose - this file is machine-produced, so an unexpected key means the producer drifted from the contract.",
  "type": "object",
  "required": [
    "schema_version",
    "type",
  ...
  "properties": {
    ...
    "prices_as_of": {
      "type": "string",
      "description": "YYYY-MM-DD, or the empty string when no source was available.",
      "pattern": "^([0-9]{4}-[0-9]{2}-[0-9]{2})?$"
    },
    "source": {
      "type": ["string", "null"],
      "description": "Machine name of the cost source, kebab-case, or null when none was available.",
      "pattern": "^[a-z0-9][a-z0-9-]*$"
    },
    "source_label": { "type": ["string", "null"], "minLength": 1 },
    "totals": {
      "type": "object",
      "required": [
        "attributed_usd",
        "shared_orchestration_usd",
        "courses_with_data",
      ...
      "properties": {
        "attributed_usd": {
          "type": "number",
          "minimum": 0,
          "description": "Sum of the rounded courses[].cost_usd. Never summed with shared_orchestration_usd."
        },
        "shared_orchestration_usd": { "type": "number", "minimum": 0 },
        "courses_with_data": { "type": "integer", "minimum": 0 },
        "courses_without_data": { "type": "integer", "minimum": 0 },
        "transcripts_scanned": { "type": "integer", "minimum": 0 },
    ...
    "courses": {
      "type": "array",
      "description": "Descending by cost_usd, then ascending by course, then ascending by dir. A course with no evidence is never here.",
      "items": {
        "type": "object",
        "required": ["course", "dir", "cost_usd", "transcripts", "requests", "scope"],
        "properties": {
          "course": { "type": "string", "minLength": 1 },
          "dir": { "type": "string", "minLength": 1 },
          "cost_usd": { "type": "number", "exclusiveMinimum": 0 },
          "transcripts": { "type": "integer", "minimum": 1 },
          "requests": { "type": "integer", "minimum": 0 },
          "scope": { "enum": ["full", "generation-only"] }
        },
        "additionalProperties": false
      }
    },
    "no_data": {
      "type": "array",
      "description": "Course directories on disk with zero credited evidence, sorted by dir - the unique key. Never rendered as a dollar figure. Objects, not bare basenames (amended 2026-08-12, adversarial review finding NEW-1): two directories in different domains can share a basename, and a bare string here could not represent both without either colliding or breaking the cross-field check below.",
      "items": {
        "type": "object",
        "required": ["course", "dir"],
        "properties": {
    ...
    "shared_orchestration": {
      "type": "object",
      "description": "Cost that belongs to no single course: sessions that wrote several, or supervised the sub-agents that did. Shown whole, never divided.",
      "required": ["cost_usd", "transcripts", "requests", "courses", "transcript_ids"],
      "properties": {
        "cost_usd": { "type": "number", "minimum": 0 },
        "transcripts": { "type": "integer", "minimum": 0 },
        "requests": { "type": "integer", "minimum": 0 },
        "courses": { "type": "array", "items": { "type": "string", "minLength": 1 } },
        "transcript_ids": { "type": "array", "items": { "type": "string", "minLength": 1 } }
      },
      "additionalProperties": false
    },
    "limits": {
      "type": "array",
      "description": "The disclosures the page renders verbatim. Never empty: the first two are unconditional.",
      "minItems": 2,
